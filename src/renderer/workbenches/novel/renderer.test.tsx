...
describe("NovelWorkbenchRenderer storage loop", () => {
  it("将当前结算阶段投影为连续的轮内结算流", () => {
    expect(buildSettlementSteps("arbitrating")).toEqual([
  ...
  it("按事件类型生成文字舞台行动，而不是复用固定选项", () => {
    expect(
      storyChoicesForEvent({ kind: "faction-strategy" } as SimulationEvent).map(
        (choice) => choice.label,
      ),
    ).toEqual(["旁观局势", "发起会商", "准备反制"]);
    expect(
      storyChoicesForEvent({ kind: "character-action" } as SimulationEvent).map(
        (choice) => choice.label,
      ),
    ).toEqual(["跟随行动", "投递线索", "保持沉默"]);
    expect(
      storyChoicesForEvent({ kind: "world-process" } as SimulationEvent).map(
        (choice) => choice.label,
      ),
    ).toEqual(["继续观测", "设置护栏", "创建分支"]);
  });

  it("将世界推演作为辅助菜单下的唯一直接入口", () => {
    ...
    expect(
      navigation.some((item) =>
        ["simulation-console", "simulation-lab", "simulation-council"].includes(
          item.id,
        ),
      ),
    ).toBe(false);
    expect(navigation.find((item) => item.id === "simulation")?.parentId).toBe(
      "utilities",
    );
  });

  it("没有运行时直接展示三栏基线工作面，并可无配置创建推演", async () => {
    const storage = createEmptyNovelStorage();
    const navigate = vi.fn();
    render(
      <NovelWorkbenchRenderer
        context={context(storage, "simulation", navigate)}
      />,
    );

    expect(
      await screen.findByRole("heading", { name: "开始一场推演" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByText("File changed externally"),
    ).not.toBeInTheDocument();
    expect(screen.queryByRole("region", { name: "本轮世界脉动" })).not.toBeInTheDocument();
    expect(screen.getByText("当前时间点")).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "创建推演" }),
    ).toBeEnabled();
    expect(screen.getByText("默认即可开始")).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "打开可选设置" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(
        "事实起点、观察范围、周期规则、人物地点和推演预算都会从当前世界架构自动推导；没有资料时也可以先推进世界时钟。",
      ),
    ).not.toBeInTheDocument();
    expect(screen.queryByText("事实截止点")).not.toBeInTheDocument();
    expect(screen.queryByText("观察地域")).not.toBeInTheDocument();
    expect(screen.queryByText("重点人物")).not.toBeInTheDocument();
    expect(screen.queryByText("重点势力")).not.toBeInTheDocument();
    expect(screen.queryByText("高级预算与传播约束")).not.toBeInTheDocument();
    expect(screen.queryByText("尚未设置事实截止点")).not.toBeInTheDocument();
    expect(screen.queryByText(/缺少初始地点/u)).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "创建推演" }));
    expect(
      await screen.findByRole("heading", { name: "事实基线" }),
    ).toBeInTheDocument();
    expect(screen.getByLabelText("下一轮结算范围")).toBeInTheDocument();
    expect(screen.getByText(/下一轮将从/)).toBeInTheDocument();
    expect(
      screen.getByText("人物会按记忆、处境和当前意图回应世界。"),
    ).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "故事从这里开始" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /推演第一轮/ })).toBeEnabled();
    fireEvent.click(screen.getByRole("button", { name: /先看人物/ }));
    expect(screen.getByRole("button", { name: "人物" })).toHaveClass("is-active");
    fireEvent.click(screen.getByRole("button", { name: /推演第一轮/ }));
    expect(
      await screen.findByRole("heading", { name: /第 1 轮世界变化/ }),
    ).toBeInTheDocument();
  });

  it("新建推演先编辑配置，取消不创建运行，确认后才进入舞台", async () => {
    const storage = createEmptyNovelStorage();
    const repository = createWorldSimulationRepositoryV2(storage);
    render(
      <NovelWorkbenchRenderer
        context={context(storage, "simulation", vi.fn())}
      />,
    );

    await screen.findByRole("heading", { name: "开始一场推演" });
    fireEvent.click(
      screen.getByRole("button", { name: /当前推演.*新建推演/u }),
    );
    fireEvent.click(screen.getByRole("button", { name: "新建" }));

    expect(
      await screen.findByRole("dialog", { name: "推演设置" }),
    ).toBeInTheDocument();
    expect((await repository.loadRunIndex()).value.runs).toHaveLength(0);
    fireEvent.change(screen.getByLabelText("推演名称"), {
      target: { value: "取消的新推演" },
    });
    fireEvent.click(screen.getByRole("button", { name: "取消" }));

    expect(screen.queryByRole("dialog", { name: "推演设置" })).not.toBeInTheDocument();
    expect((await repository.loadRunIndex()).value.runs).toHaveLength(0);
    expect(
      (await repository.loadScenarios()).value.scenarios.some(
        (scenario) => scenario.name === "取消的新推演",
      ),
    ).toBe(false);
    ...
    fireEvent.click(screen.getByRole("button", { name: "新建" }));
    fireEvent.change(await screen.findByLabelText("推演名称"), {
      target: { value: "已配置的新推演" },
    });
    fireEvent.click(
      screen.getByRole("button", { name: "保存并进入舞台" }),
    );
    ...
    await waitFor(async () => {
      const index = await repository.loadRunIndex();
      expect(index.value.runs).toHaveLength(1);
      expect(index.value.runs[0]?.name).toBe("已配置的新推演");
    });
  });

  it("点击检查器实体只改变状态焦点，不改变当前轮次和事件", async () => {
    const storage = createEmptyNovelStorage();
    const repository = createWorldSimulationRepositoryV2(storage);
    const scenarios = await repository.loadScenarios();
    const scenario = scenarios.value.scenarios[0];
    if (!scenario) throw new Error("缺少默认推演方案");
    const character = (id: string, name: string): CharacterProjection => ({
      id,
      name,
      summary: `${name}的日常状态`,
      status: "在世",
    ...
    const baseline = {
      ...
      baselineId: "baseline-inspector-focus",
      projectId: "novel-test",
      projectTitle: "检查器焦点测试",
      sourceRevision: "inspector-focus-revision",
      ...
      characters: [character("character-shen", "沈砚"), character("character-lu", "陆离")],
      factions: [],
      regions: [],
      items: [],
      rules: [],
    ...
    const initial = createWorldSimulationRun(baseline, scenario);
    const main = initial.branches[0];
    if (!main) throw new Error("缺少主推演分支");
    const nextState = {
      ...main.state,
      currentTime: {
        ...main.state.currentTime,
        sortKey: "1",
        displayText: "第 1 日",
      },
    };
    const event: SimulationEvent = {
      id: "event-inspector-focus",
      sequence: 1,
      time: {
        calendarId: "cosmic",
        sortKey: "1",
    ...
    expect((await screen.findAllByText("沈砚观察局势")).length).toBeGreaterThan(0);
    const stateCards = document.querySelectorAll(".ws4-state-list article");
    expect(stateCards).toHaveLength(2);
    fireEvent.click(stateCards[1]!);
    expect(screen.getByText("状态焦点")).toBeInTheDocument();
    expect(screen.getAllByText("陆离").length).toBeGreaterThan(0);
    expect(screen.getAllByText("沈砚观察局势").length).toBeGreaterThan(0);
  });

  it("按轮次回看对应状态快照，并在主操作区设置连续推演轮数", async () => {
    const storage = createEmptyNovelStorage();
    const repository = createWorldSimulationRepositoryV2(storage);
    const scenario = (await repository.loadScenarios()).value.scenarios[0];
    if (!scenario) throw new Error("缺少默认推演方案");
    const baseline = {
      schemaVersion: WORLD_SIMULATION_SCHEMA_VERSION,
      baselineId: "baseline-renderer-snapshot",
      projectId: "novel-test",
      projectTitle: "测试小说",
    ...
    await repository.createRun(createWorldSimulationRun(baseline, scenario));
    render(
      <NovelWorkbenchRenderer
        context={context(storage, "simulation", vi.fn())}
      />,
    ...
    const advance = await screen.findByRole("button", { name: "推演一轮" });
    await waitFor(() => expect(advance).toBeEnabled());
    fireEvent.change(screen.getByLabelText("连续推演轮数"), {
      target: { value: "2" },
    });
    expect(screen.getByLabelText("连续推演轮数")).toHaveValue(2);
    expect(screen.getByText("本分支报告")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "生成报告" })).toBeEnabled();
    expect(
      screen.getByRole("button", { name: /第 0 轮 · 事实基线/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "事实基线" }),
    ).toBeInTheDocument();
    const pendingRound = screen.getByRole("button", {
      name: /第 1 轮.*待推演/u,
    });
    expect(pendingRound).toBeInTheDocument();
    fireEvent.click(pendingRound);
    expect(
      screen.getByRole("heading", { name: "等待下一次结算" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: "本轮世界脉动" }),
    ).toBeInTheDocument();
    expect(screen.getByText(/下一轮将从/u)).toBeInTheDocument();
    expect(
      screen.getByText("传播本轮事件，保存新的状态检查点"),
    ).toBeInTheDocument();
    fireEvent.click(
      screen.getAllByRole("button", { name: "查看当前状态" })[0]!,
    );
    expect(
      within(document.querySelector(".ws4-inspector-tabs")!).getByRole(
        "button",
        { name: "地域" },
      ),
    ).toHaveClass("is-active");
    fireEvent.click(
      screen.getByRole("button", { name: /第 0 轮 · 事实基线/u }),
    );

    fireEvent.click(advance);
    await screen.findByRole("button", { name: /第 1 轮/ });
    await waitFor(() =>
      expect(screen.getByText("第 1 轮结束状态")).toBeInTheDocument(),
    );
    expect(
      screen.getByRole("heading", { name: "这一轮没有显著事件" }),
    ).toBeInTheDocument();
    expect(screen.getByText(/个已接受变化/u)).toBeInTheDocument();
    expect(screen.getByText(/跨度 .* 观察/u)).toBeInTheDocument();
    expect(screen.getByText("当前时间点")).toBeInTheDocument();
    expect(screen.getByText("状态快照")).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: /推演一轮/u }));
    await screen.findByRole("button", { name: /第 2 轮/ });
    await waitFor(() =>
      expect(screen.getByText("第 2 轮结束状态")).toBeInTheDocument(),
    );
    ...
    expect(
      await screen.findByRole("dialog", { name: "本轮因果链" }),
    ).toBeInTheDocument();
    expect(screen.getByText("请选择一条事件查看因果链")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "关闭运行操作" }));
    expect(
      screen.queryByRole("dialog", { name: "本轮因果链" }),
    ).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "更多运行操作" }));
    expect(
      await screen.findByRole("dialog", { name: "更多运行操作" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "生成本分支报告" }),
    ).toBeEnabled();
    fireEvent.click(screen.getByRole("button", { name: "关闭运行操作" }));
    fireEvent.click(screen.getByRole("button", { name: /当前分支/u }));
    fireEvent.click(screen.getByRole("button", { name: "自然对照" }));
    await screen.findAllByText("自然演化对照");
    fireEvent.click(screen.getByRole("button", { name: "更多运行操作" }));
    expect(screen.getByText("分支对照")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "关闭运行操作" }));

    const workspaceBody = document.querySelector(".ws4-body");
    expect(workspaceBody).toHaveClass("is-mobile-stage");
    fireEvent.click(screen.getByRole("button", { name: "轮次" }));
    expect(workspaceBody).toHaveClass("is-mobile-timeline");
    fireEvent.click(screen.getByRole("button", { name: "状态" }));
    expect(workspaceBody).toHaveClass("is-mobile-inspector");
  });

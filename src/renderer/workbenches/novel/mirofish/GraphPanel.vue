<template>
  <div class="graph-panel">
    <div class="panel-header">
      <span class="panel-title">世界关系图谱</span>
      ...
      <div class="header-tools">
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" title="刷新图谱">
          ...
          <span class="btn-text">刷新图谱</span>
        </button>
        <button class="tool-btn" @click="$emit('toggle-maximize')" title="最大化/还原">
          <span class="icon-maximize" aria-hidden="true">全屏</span>
        </button>
      </div>
    </div>
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="selectedItem" class="detail-panel">
          <div class="detail-panel-header">
            ...
            <button class="detail-close" @click="closeDetailPanel" title="关闭详情">关闭</button>
          </div>
          
          <!-- 节点详情 -->
          <div v-if="selectedItem.type === 'node' && selectedItem.data" class="detail-content">
          ...
          <div v-else class="detail-content">
                ...
                >
                  ...
                  <div class="self-loop-item-content" v-show="expandedSelfLoops.has(loop.uuid || idx)">
                    ...
                    <div v-if="loop.episodes && loop.episodes.length > 0" class="self-loop-episodes">
                      <span class="detail-label">来源片段：</span>
            ...
            <template v-else>
              <div class="edge-relation-header">
                {{ selectedItem.data.source_name }} → {{ selectedItem.data.name || '相关' }} → {{ selectedItem.data.target_name }}
              </div>
              ...
              <div class="detail-row">
                ...
                <span class="detail-value">{{ selectedItem.data.name || '相关' }}</span>
              </div>
              <div class="detail-row">
                <span class="detail-label">类型：</span>
              ...
              <div class="detail-section" v-if="selectedItem.data.episodes && selectedItem.data.episodes.length > 0">
                <div class="section-title">来源片段</div>
...
// 高亮节点联动
watch(() => props.highlightNodeUuid, (uuid) => {
  if (!graphSvg.value) return
  const svg = d3.select(graphSvg.value)
  // 重置所有节点和标签
  svg.selectAll('.nodes circle').attr('stroke', cssVar('--bg-primary', '#fff')).attr('stroke-width', 2.5)
  svg.selectAll('.nodes circle').attr('filter', null)
  svg.selectAll('.nodes text').attr('font-weight', '500').attr('fill', cssVar('--text-primary', '#303133'))

  if (uuid) {
    // 高亮目标节点：边框 + glow + 标签加粗变色
    svg.selectAll('.nodes circle').each(function (d) {
      if (d.uuid === uuid) {
        d3.select(this)
          .attr('stroke', cssVar('--primary-color', '#5470c6'))
          .attr('stroke-width', 3)
          .attr('filter', 'url(#glow-filter)')
        // 高亮对应的标签
        svg.selectAll('.nodes text').each(function (t) {
          if (t.uuid === uuid) {
            d3.select(this)
              .attr('font-weight', '700')
              .attr('fill', cssVar('--primary-color', '#5470c6'))
          }
        })
      }
    })
...
// 计算实体类型用于图例
// 读取 CSS 变量值（用于 D3 SVG 动态着色）
const cssVar = (name, fallback = '#999') => {
...
const entityTypes = computed(() => {
  ...
  props.graphData.nodes.forEach(node => {
    ...
    if (!typeMap[type]) {
      typeMap[type] = { name: type, count: 0, color: CATEGORY_COLORS[Object.keys(typeMap).length % CATEGORY_COLORS.length] }
    }
    typeMap[type].count++
  })
...
const renderGraph = () => {
  ...
  const link = linkGroup.selectAll('path')
    .data(edges)
    .enter().append('path')
    .attr('stroke', cssVar('--border-primary', '#e4e7ed'))
    ...
    .on('click', (event, d) => {
      ...
      // 重置之前选中边的样式
      linkGroup.selectAll('path').attr('stroke', cssVar('--border-primary', '#e4e7ed')).attr('stroke-width', 1.5)
      linkLabelBg.attr('fill', 'rgba(255,255,255,0.95)')
      linkLabels.attr('fill', cssVar('--text-secondary', '#909399'))
      // 高亮当前选中的边
      d3.select(event.target).attr('stroke', cssVar('--color-info', '#3498db')).attr('stroke-width', 3)
  ...
  const linkLabelBg = linkGroup.selectAll('rect')
    ...
    .on('click', (event, d) => {
      ...
      linkLabels.attr('fill', cssVar('--text-secondary', '#909399'))
      // 高亮对应的边
      link.filter(l => l === d).attr('stroke', cssVar('--color-info', '#3498db')).attr('stroke-width', 3)
  ...
  const linkLabels = linkGroup.selectAll('text')
    ...
    .text(d => d.name)
    .attr('font-size', '9px')
    .attr('fill', cssVar('--text-secondary', '#909399'))
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .attr('stroke', cssVar('--bg-primary', '#fff'))
    ...
    .on('click', (event, d) => {
      event.stopPropagation()
      // 重置所有节点样式
      node.attr('stroke', cssVar('--bg-primary', '#fff')).attr('stroke-width', 2.5)
      linkGroup.selectAll('path').attr('stroke', cssVar('--border-primary', '#e4e7ed')).attr('stroke-width', 1.5)
      // 高亮选中节点
    ...
    .on('mouseleave', (event, d) => {
      if (!selectedItem.value || selectedItem.value.data?.uuid !== d.rawData.uuid) {
        d3.select(event.target).attr('stroke', cssVar('--bg-primary', '#fff')).attr('stroke-width', 2.5)
      }
    })
  ...
  const nodeLabels = nodeGroup.selectAll('text')
    ...
    .attr('font-size', '11px')
    .attr('fill', cssVar('--text-primary', '#303133'))
...
.panel-header {
  ...
  align-items: center;
  background: color-mix(in srgb, var(--paper-elevated, #fff) 94%, transparent);
  pointer-events: none;
}
...
.graph-legend {
  ...
  bottom: 24px;
  left: 24px;
  background: color-mix(in srgb, var(--paper-elevated, #fff) 96%, transparent);
  padding: 12px 16px;
  border-radius: 8px;
...
.slider {
  ...
  right: 0;
  bottom: 0;
  background-color: var(--line-strong, #c9ced6);
  border-radius: 22px;
  transition: 0.3s;
}
...
.slider:before {
  ...
  left: 3px;
  bottom: 3px;
  background-color: var(--paper-elevated, #fff);
  border-radius: 50%;
  transition: 0.3s;
}

input:checked + .slider {
  background-color: var(--accent-cool, #3c6e9e);
}
...
.detail-panel-header {
  ...
  border-bottom: 1px solid var(--line-subtle, #eaedf0);
  flex-shrink: 0;
}

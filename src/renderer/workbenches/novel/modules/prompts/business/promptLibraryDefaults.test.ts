...
describe("StoryForge prompt-library defaults", () => {
  it("contains one versioned My Novel Studio installation", () => {
  ...
  it("preserves all 89 StoryForge prompts and the人物库 Agent prompt", () => {
    ...
    expect(storyForgePrompts).toHaveLength(
      STORYFORGE_PROMPT_COUNT + NOVEL_WORKBENCH_PROMPT_COUNT,
    );
    expect(storyForgePrompts.filter((prompt) => prompt.enabled)).toHaveLength(
      41,
    );
    expect(storyForgePrompts.filter((prompt) => !prompt.enabled)).toHaveLength(
      49,
    ...
    expect(storyForgePrompts[0]?.content).toContain(
      '"sourceProject": "My Novel Studio"',
    );
    const bookEditor = storyForgePrompts.find(
      (prompt) => prompt.name === "内置-全书编辑",
    );
    expect(bookEditor?.content).toContain(
      "你是 My Novel Studio 的全书编辑 Agent",
    );
    expect(bookEditor?.content).not.toContain(
  ...
  it("builds a valid single-root directory tree with mapped genre scopes", () => {
    ...
    expect(
      roots.find(
        (root) => root.skillPackId === STORYFORGE_PROMPT_INSTALLATION_ID,
      ),
    ).toEqual(
      expect.objectContaining({
        nodeKind: "pack-root",

...
export function useNovelProject(
  ...
  const load = useCallback(
    async (background: boolean): Promise<LoadedNovelProject | null> => {
      ...
      try {
        ...
        // 同步未写盘时首次加载结果已是最终状态，跳过第二次全量读取。
        const synchronized = await repository.synchronizeNarrative(
          loaded,
          loaded.chapterIndex.structureMode,
        );
        if (synchronized.changed) loaded = await repository.load();
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        let loaded = await repository.load();
        // 与首次加载保持一致：稳定态同步不写盘时，首次读取即为最终状态。
        const synchronized = await repository.synchronizeNarrative(
          loaded,
          loaded.chapterIndex.structureMode,
        );

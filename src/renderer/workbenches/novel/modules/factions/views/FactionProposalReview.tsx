...
function factionSummary(value: unknown): string {
  const faction = value as Partial<FactionRecord>;
  const parts = [
    faction.type,
    faction.status === "active" ? "活跃" : faction.status,
    faction.summary,
  ].filter(Boolean);
  return parts.join(" · ");
}
...
function OperationCard({
  ...
  readonly operation: FactionProposalOperation;
  readonly onToggle: () => void;
  readonly onApply: () => void;
  readonly onReject: () => void;
  readonly acting: boolean;
}) {
  const faction = operation.value as Partial<FactionRecord>;
  return (
    <article
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-xs leading-5 text-[var(--ink-muted)]">
            {factionSummary(operation.value)}
          </p>
          <div className="mt-2 flex flex-wrap gap-2 text-xs text-[var(--ink-subtle)]">
            <span>
              {faction.members?.length ?? 0} 成员 ·{" "}
              {faction.territories?.length ?? 0} 领地 ·{" "}
              {faction.resources?.length ?? 0} 资源
            </span>
            {faction.links && faction.links.length > 0 && (
              <span>{faction.links.length} 链接</span>
            )}
          </div>
        </div>
        {operation.status === "pending" && (
...
export default function FactionProposalReview({
  ...
  const [proposals, setProposals] = useState<LoadedFactionProposal[]>([]);
        ...
        <div className="space-y-4">
          {proposals.map((proposal) => {
            const proposalSelected = proposal.manifest.operations.filter(
              (operation) =>
                operation.status === "pending" && selected.has(operation.candidateId),
            );
            return (
              <section
              ...
              >
                <header className="flex flex-wrap items-center gap-3 border-b border-[var(--line-subtle)] px-4 py-3">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void run(() =>
                        repository.deleteProposals([proposal.manifest.proposalId]),
                      )
                    }
                  >

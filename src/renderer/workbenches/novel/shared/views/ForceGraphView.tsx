...
export interface ForceGraphViewProps {
  ...
  readonly edges: readonly ForceGraphEdge[];
  ...
  readonly detailsPosition?: "left" | "right";
  readonly emptyDetailsText?: string;
  readonly interactionHint?: string;
  readonly openNodeLabel?: string;
  readonly canOpenNode?: (node: ForceGraphNode) => boolean;
  readonly onOpenNode?: (node: ForceGraphNode) => void;
}

/** 小说工作台共享的只读力导向图谱，提供检查器、中心模式与键盘导航。 */
export default function ForceGraphView({
  ...
  nodeColors,
  ariaLabel,
  detailsTitle = "节点详情与关系",
  detailsPosition = "right",
  emptyDetailsText = "点击图谱中的节点查看详情；双击节点以其为中心展开邻居。",
  interactionHint = "单击/方向键选中 · 双击设为中心 · 拖拽调整",
  openNodeLabel,
  canOpenNode,
  onOpenNode,
}: ForceGraphViewProps) {
  ...
  useEffect(() => {
    ...
    // D3 与 SVG 必须使用同一逻辑坐标系；DOM 像素尺寸会由 viewBox 自适应。
    ...
    node
      .append("circle")
      .attr("r", 9)
      .attr("fill", (datum) => nodeColors[datum.kind] ?? "#888")
    ...
    link
      ...
      .text(
        (datum) =>
          `${datum.label}：${(datum.source as GraphNodeDatum).label} ↔ ${(datum.target as GraphNodeDatum).label}`,
      );
  ...
  }, [nodeColors, onSelect, visible, visibleEdges]);
  ...
  const canOpenSelected =
    ...
    openNodeLabel &&
    onOpenNode &&
    (canOpenNode?.(selectedNode) ?? true);

  return (
    <div
      className={`flex min-h-0 flex-1 ${detailsPosition === "left" ? "flex-row-reverse" : ""}`}
    >
        ...
        <svg
          ...
          onKeyDown={(event) => {
            ...
            if (["ArrowDown", "ArrowRight"].includes(event.key)) {
              ...
              const nextIndex = (focusIndex + 1) % visible.length;
              setFocusIndex(nextIndex);
              onSelect(visible[nextIndex].id);
            } else if (["ArrowUp", "ArrowLeft"].includes(event.key)) {
              event.preventDefault();
              const nextIndex =
                (focusIndex - 1 + visible.length) % visible.length;
              setFocusIndex(nextIndex);
              onSelect(visible[nextIndex].id);
            } else if (event.key === "Enter") {
              event.preventDefault();
              onSelect(visible[focusIndex % visible.length].id);
            }
          }}
        />
        {showCollapsed && (
      ...
      <aside
        className={`flex w-72 shrink-0 flex-col border-[var(--line-subtle)] max-lg:hidden ${detailsPosition === "left" ? "border-r" : "border-l"}`}
      >
                  ...
                  {selectedNeighbors.map(({ edge, other }) => (
                    <li key={edge.id}>
                      ...
                      >
                        ...
                        {edge.description && (
                          <span className="mt-1 block line-clamp-3 leading-5 text-[var(--ink-subtle)]">
                            {edge.description}
                          </span>
                        )}
                      </button>

...
/**
 * Build the Chat shape for replacing the current tab with a persisted history
 * session. Workbench agent surfaces are host-owned runtime projections, not
 * properties of the persisted transcript, so none of their session-scoped
 * state may cross this boundary.
 */
export function buildHistoryChatFlipPatch(
  tab: Tab,
  fields: {
    agentDir: string;
    sessionId: string;
  ...
  const historyTargetTab = { ...tab };
  delete historyTargetTab.workbench;
  delete historyTargetTab.workbenchAgentSurface;
  delete historyTargetTab.sessionHistoryGroupPath;
  delete historyTargetTab.initialMessage;
  delete historyTargetTab.pendingFilePreview;

  return {
    ...buildChatFlipPatch(historyTargetTab, fields),
    // These flags describe the replaced source session rather than the
    // selected transcript. Let the newly mounted Chat establish them.
    isGenerating: false,
    hasUnread: false,
  };
}

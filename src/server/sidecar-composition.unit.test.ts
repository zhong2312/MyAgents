...
describe("Sidecar production composition", () => {
  it("keeps omitted role fail-safe Session and requires an explicit development union", () => {
    expect([...resolveSidecarComposition(null, false).capabilities]).toEqual([
      "common",
      "session",
    ]);
    expect([...resolveSidecarComposition(null, true).capabilities]).toEqual([
      "common",
      "global",
      "session",
    ]);
  ...
  it.each([
    ...
    [
      "POST",
      "/api/workbench-ai/run/7ed0f6ee-0000-4000-8000-000000000000/cancel",
      "global",
    ],
  ...
  it("does not grant unknown control routes a default capability", () => {
    expect(
      classifySidecarRequest(request("/api/admin/future-owner", "POST")),
    ).toBeNull();
    expect(
  ...
  it.each([
    ...
    "%s rejects wrong-role %s %s before the real handler",
  ...
  it("runs common plus only the selected production boot capability", async () => {
    ...
    const steps = (["common", "global", "session"] as const).map(
      (capability) => ({
        capability,
        run: () => {

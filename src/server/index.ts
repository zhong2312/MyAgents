...
const CRASH_LOG_FILE = (() => {
  try {
    ...
    /* fall through; later writes will retry */
  }
...
function evictOldCrashLogs(): void {
  try {
    ...
    const entries = readdirSync(CRASH_LOG_DIR)
      ...
      .filter(
        (x): x is { path: string; mtimeMs: number; size: number } => x !== null,
      )
...
const dumpFingerprints = new Map<
  string,
  { count: number; firstSeen: number; suppressed: boolean }
>();
...
function fingerprintError(err: unknown): string {
  ...
  if (err instanceof Error) {
    ...
    const stackHead = (err.stack ?? err.message ?? "")
      .split("\n")
      .slice(0, 2)
      .join("|")
      .slice(0, 200);
...
function shouldDumpContextFor(err: unknown): boolean {
  ...
  if (dumpFingerprints.size > 50) {
    for (const [k, v] of dumpFingerprints) {
      if (now - v.firstSeen > CRASH_DEDUPE_WINDOW_MS)
        dumpFingerprints.delete(k);
    }
  }
  ...
  if (!entry.suppressed) {
    ...
    appendFileSyncSafely(
      `[${new Date().toISOString()}] SUPPRESS_CONTEXT fingerprint=${fp.slice(0, 100)} count=${entry.count} — further dumps for this fingerprint suppressed for the next ${CRASH_DEDUPE_WINDOW_MS / 1000}s\n`,
    );
  }
  return false;
}
...
function appendFileSyncSafely(line: string): void {
  ...
  try {
    appendFileSync(CRASH_LOG_FILE, line);
  } catch {
    /* ignore */
  }
}
...
function dumpCrashContext(reason: string, errForFingerprint?: unknown): void {
  if (crashLogCeilingHit) return;
  if (
    errForFingerprint !== undefined &&
    !shouldDumpContextFor(errForFingerprint)
  )
    return;
  try {
    ...
    appendFileSync(
      CRASH_LOG_FILE,
      banner + lines.join("") + "--- end crash context ---\n",
    );
...
function goalContinuationScenarioForSession(
  ...
  if (origin?.kind === "agent-channel") {
    ...
    const sourceType: "private" | "group" =
      tail === "group" ? "group" : "private";
    const platform =
      parts.length > 1 ? parts.slice(0, -1).join("_") : parts[0] || "unknown";
    return {
      type: "agent-channel",
      platform,
      sourceType,
...
function getRuntimeConfigPermissionMode(
  ...
  return permissionMode
    ? coercePermissionModeForRuntime(permissionMode, runtime)
    : undefined;
}
...
function getRuntimeConfigSource(
  ...
  return source === "managed-provider" || source === "system-cli"
    ? source
    : undefined;
}
...
function resolveCronProviderRouting(
  ...
  if (provider.type === "subscription") {
    return (
      (resolveProviderEnv(providerId) as ProviderEnv | undefined) ??
      "subscription"
    );
  }
  ...
  const guard: import("./session-core/turn-queue").DispatchGuard = async () => {
    ...
    return response.ok === true
      ...
      : {
          ...
          error: String(
            response.error ?? "Task execution is no longer authorized",
          ),
        };
  };
  guard.cancel = () => {
...
function requiredMemorySystemSkill(
  managedKind: string | undefined,
): RequiredMemorySystemSkill | undefined {
  switch (managedKind) {
    case "memory_auto_update_batch":
      return "myagents-memory-update";
    case "memory_gardener":
      return "myagents-memory-gardener";
    case "memory_molt":
      return "myagents-memory-molt";
    default:
      return undefined;
  }
}
...
function systemMaintenanceKindFromCronPayload(
  payload: CronExecutePayload,
): SystemMaintenanceSessionKind | undefined {
...
async function handleGoalExecuteSync(request: Request): Promise<Response> {
  ...
  if (
    ...
    return jsonResponse(
      { success: false, error: "Invalid Goal execution payload." },
      400,
    );
  }
  ...
  return withScheduledTurnDispatchLock(async () => {
    ...
    try {
          ...
          return jsonResponse(
            {
              success: false,
              error:
                result.error ?? "Goal execution termination was not confirmed",
      ...
      console.error(
        `[goal] execute-sync goalId=${payload.goalId} failed:`,
        error,
      );
      return failure(
 ...
 */
function redactSessionMetadata<T extends { providerEnvJson?: string }>(
  meta: T,
): T {
...
async function routeAdminApi(
  ...
  // Agent commands
  if (route === "agent/list")
    return api.handleAgentList(
      payload as Parameters<typeof api.handleAgentList>[0],
    );
  if (route === "agent/show")
    return api.handleAgentShow(
  ...
  if (route === "space/issue-delivery-ignore")
    return await api.handleSpaceIssueDeliveryIgnore(
      payload as Parameters<typeof api.handleSpaceIssueDeliveryIgnore>[0],
    );
  ...
  async function handleRequest(request: Request): Promise<Response> {
    {
      ...
      if (pathname === "/chat/send" && request.method === "POST") {
        ...
        try {
          ...
          const runtimeLabel =
            engine.kind === "external" ? getActiveRuntimeType() : "builtin";
          console.log(
            `[chat] send via ${runtimeLabel}: text="${text.slice(0, 200)}" images=${images.length} mode=${permissionMode} model=${model ?? "default"} baseUrl=${providerLabel}`,
          );
      ...
      if (pathname === "/cron/execute-sync" && request.method === "POST") {
        ...
        return await withScheduledTurnDispatchLock(async () => {
          ...
          if (payload.initializeSession) {
            ...
            const taskAgent = findAgentByWorkspacePath(agentDir) as
              | AgentConfig
              | undefined;
            const engine = getSessionEngine();
            ...
            const overrideRuntimeSource =
              overrideRuntimeType === "builtin"
                ? undefined
            ...
            const agentSnapshot: Partial<SessionMetadata> = taskAgent
              ? snapshotForOwnedSession(taskAgent, {
                  ...
                  ...(overrideRuntimeSource
                    ? { runtimeSourceOverride: overrideRuntimeSource }
                    : {}),
                  managedCodexProviderReady:
                    overrideRuntimeSource === "managed-provider"
                      ? true
                      : managedCodexReady,
                })
              : {};
            const taskSnapshot = bindOwnedSnapshotToRuntimeIdentity(
              agentSnapshot,
              liveRuntimeIdentity,
            );
            ...
            const systemMaintenanceKind =
              systemMaintenanceKindFromCronPayload(payload);
            if (systemMaintenanceKind) {
              taskSnapshot.systemMaintenanceKind = systemMaintenanceKind;
            }
            const runtimeBackedIdentity =
              runtimeBackedProviderIdentityFromCronRuntime(
                overrideRuntimeType,
                overrideRuntimeSource,
            ...
            if (!switched) {
              console.error(
                `[cron] execute-sync taskId=${taskId} failed to switch to new session ${newSession.id}`,
              );
              return jsonResponse(
                {
                  success: false,
            ...
            console.log(
              `[cron] execute-sync taskId=${taskId} new_session mode: created fresh session ${newSession.id} (from=${sessionId ? "rust-payload" : "bun-fallback"})`,
            );
          } else if (sessionId) {
            ...
            if (currentSessionId === sessionId) {
              console.log(
                `[cron] execute-sync taskId=${taskId} existing session: already in ${sessionId}, skipping switch`,
              );
            } else {
              console.log(
                `[cron] execute-sync taskId=${taskId} attempting to switch to session ${sessionId}`,
              );
              const switched = await switchToSession(sessionId);
              if (!switched) {
                console.error(
                  `[cron] execute-sync taskId=${taskId} failed to switch to required session ${sessionId}`,
                );
                return jsonResponse(
                  {
                    success: false,
                ...
                console.log(
                  `[cron] execute-sync taskId=${taskId} existing session: switched to ${sessionId}`,
                );
              }
            }
            ...
            console.log(
              `[cron] execute-sync taskId=${taskId} no sessionId provided, using current session`,
            );
          }
          ...
          if (payload.providerId) {
            ...
            const sessionMeta = snapshotSessionId
              ...
              : undefined;
            const agent = findAgentByWorkspacePath(agentDir) as
              | AgentConfig
              | undefined;
            if (sessionMeta && agent) {
              ...
              if (isConcreteProviderRoute(resolved.providerRoute)) {
                ...
                if (!decoded) {
                  const message =
                    resolved.providerId &&
                    isProviderDisabled(resolved.providerId)
              ...
              if (resolved.runtime !== "builtin") {
                effectiveRuntimeConfig = {
                  ...
                  source:
                    payload.runtimeConfig?.source ??
                    resolved.runtimeSource ??
                    effectiveRuntimeConfig?.source,
                  model: payload.runtimeConfig?.model ?? resolved.model,
                  permissionMode:
                    payload.runtimeConfig?.permissionMode ??
                    payload.permissionMode ??
          ...
          if (
            getRuntimeConfigSource(effectiveRuntimeConfig ?? null) ===
              "managed-provider" &&
            !managedCodexReady
          ) {
            const errMsg = managedCodexNotReadyMessage("cron task execution");
            console.error(
              `[cron] execute-sync managed Codex runtimeConfig rejected: ${errMsg}`,
            );
            clearCronTaskContext(effectiveSessionId);
            resetInteractionScenario();
          ...
          console.log(
            `[cron] execute-sync: cron context set for taskId=${taskId}`,
          );
          ...
          try {
            ...
            console.error(
              "[cron] execute-sync: error setting interaction scenario",
              e,
            );
            clearCronTaskContext(effectiveSessionId);
            return jsonResponse(
              { success: false, error: `System prompt error: ${e}` },
              500,
            );
          }
          ...
          try {
            console.log(
              `[cron] execute-sync taskId=${taskId} runMode=${effectiveRunMode} interval=${intervalMinutes}min exec#${executionNumber} aiCanExit=${aiCanExit ?? false} prompt="${prompt.slice(0, 100)}..."`,
            );
            ...
            const engine = getSessionEngine();
            const cronRuntimeType: RuntimeType =
              engine.kind === "external" ? getActiveRuntimeType() : "builtin";
            ...
            if (engine.kind === "builtin" && payload.initializeSession) {
              ...
              if (payload.mcpEnabledServers !== undefined) {
                ...
                const overrideIds = new Set(
                  payload.mcpEnabledServers.filter((id) =>
                    globalEnabledIds.has(id),
                  ),
                );
                ...
                const fromCurrent = (getCurrentMcpServers() ?? []).filter((s) =>
                  overrideIds.has(s.id),
                );
            ...
            const injectedTurn = {
              ...
              model:
                engine.kind === "external"
                  ...
                  : effectiveModel,
              providerRoute:
                engine.kind === "builtin" ? effectiveProviderRoute : undefined,
              providerEnv:
                engine.kind === "builtin" ? effectiveProviderEnv : undefined,
            ...
            const requiredSkill = requiredMemorySystemSkill(
              payload.managedKind,
            );
            const turnResult = await engine.runInjectedTurn({
              ...
              beforeDispatch: requiredSkill
                ? createRequiredSystemSkillDispatchGuard(
                    requiredSkill,
                    agentDir,
                    taskDispatchGuard,
                  )
            ...
            if (!turnResult.success) {
              console.warn(
                `[cron] execute-sync taskId=${taskId} failed via ${engine.kind}: ${turnResult.error ?? "Unknown error"}`,
              );
              clearCronTaskContext(effectiveSessionId);
              resetInteractionScenario();
              return jsonResponse(
            ...
            if (textContent) {
              const completionMatch = textContent.match(
                CRON_TASK_COMPLETE_PATTERN,
              );
              if (completionMatch) {
            ...
            console.log(
              `[cron] execute-sync taskId=${taskId} completed, aiRequestedExit=${aiRequestedExit}, exitReason=${exitReason}`,
            );
            ...
            console.log(
              `[cron] execute-sync taskId=${taskId} returning response:`,
            ...
            console.log(
              `[cron] execute-sync taskId=${taskId} returning error response:`,
      ...
      if (pathname === "/sessions" && request.method === "POST") {
        ...
        const agent = findAgentByWorkspacePath(agentDirValue) as
          | AgentConfig
          | undefined;
        ...
        if (payloadPermissionMode !== undefined)
      ...
      if (pathname.startsWith("/sessions/") && request.method === "DELETE") {
        ...
        const deleted = await deleteSession(
          sessionId,
          (current) => !isSystemMaintenanceSession(current),
        );
        if (!deleted) {
          return jsonResponse(
            { success: false, error: "Session not found." },
      ...
      if (pathname.startsWith("/sessions/") && request.method === "PATCH") {
        ...
        for (let attempt = 0; attempt < 2; attempt += 1) {
          ...
          const baseSnapshot = existingMeta.configSnapshotAt
            ? undefined
            : (() => {
                const agent = findAgentByWorkspacePath(
        ...
        return jsonResponse({
          success: true,
          session: redactSessionMetadata(updated),
        });
      }
      ...
      if (pathname === "/api/workbench-ai/run" && request.method === "POST") {
        try {
          ...
          if (payload.prompt.length > 60_000) {
            return jsonResponse(
              { success: false, error: "prompt exceeds 60000 characters." },
              400,
            );
          }
          if (
          ...
          const provider = findEffectiveProvider(providerId);
          if (!provider || provider.enabled === false) {
            return jsonResponse(
              { success: false, error: `Provider "${providerId}" is unavailable.` },
              400,
            );
          }
          const providerEnv = resolveProviderEnv(providerId);
        ...
        try {
          ...
          if (server.type === "stdio" && server.command) {
            ...
            if (server.isBuiltin && command === "npx") {
              const { getBundledNodeDir, getSystemNpxPaths, findExistingPath } =
                await import("./utils/runtime");
              ...
              if (systemNpx) {
                ...
                console.log(
                  `[api/mcp/enable] Warming up with system npx: ${warmupArgs.join(" ")}`,
                );
              } else if (nodeDir) {
                ...
                console.log(
                  `[api/mcp/enable] Warming up with bundled npx: ${warmupArgs.join(" ")}`,
                );
              } else {
          ...
          return jsonResponse({
            ...
            metadata: result.metadata
              ? redactSessionMetadata(
                  result.metadata as { providerEnvJson?: string },
                )
              : undefined,
          });
        } catch (error) {
    ...
    try {
      ...
      if (initialSessionId) {
        ...
        if (startupEngine.kind === "external") {
          ...
          setDeferredInitPhase(currentInitPhase);
          initPhaseStarted = nowMs();
          startupEngine.restoreInitialSession(
            initialSessionId,
            currentAgentDir,
          );

...
> 版本：V5
> 
> 适用范围：MyAgents 小说工作台的修行生态领域模型、编辑界面、审查规则与 V4 迁移。
...
修行生态保存在 `world/cultivation-ecology.json`，由修行生态 Repository、Schema 与 JSON codec 统一管理。JSON 保存稳定 ID、排序、引用与结构化规则；长篇理论、世界观说明与叙事备注作为字段保存在同一领域事实源中。
...
| V4 对象                           | V5 目标          | 迁移原则                                   |
| ------------------------------- | -------------- | -------------------------------------- |
| `worldOrigins`                 | 世界本源           | 保留世界本源本体、分化节点、法则、能量 / 权柄 / 信息显化及其关系。 |
| `systems`                       | 修行体系           | 更改领域名称，保留体系所有权边界。                      |
| `projections`                   | 体系本源投影         | 增加来源角色、权重、接入与副作用表达。                    |
| `mediums`                       | 理论共有节点 / 法门运行拓扑介质 | 介质与节点进入体系理论模型；具体流向、转换和损耗迁移到所属法门运行拓扑，不再作为松散顶层列表。 |
| `principles`                    | 理论模型或体系约束      | 按“解释为何成立”与“限制什么行为”分流。                  |
| `conditions`                    | 投影准入、根基要求或体系约束 | 按语义迁移，避免保留杂项条件集合。                      |
| `tracks`、`levels`、`transitions` | 成长轨道、阶段、突破与转换  | 保留并强化指标、关联和失败语义。                       |
| `resources`                     | 资源库            | 增加适用范围、最佳阶段和兼容约束。                      |
| `methods`                       | 修行法门库          | 增加运行指令、法门运行拓扑、三类上限和阶段课程。               |
| `abilities`                     | 能力库            | 拆分自动解锁与秘籍/法门修炼，增加辅助/精神/进攻类型、训练资源和释放消耗。 |
| `formations`                    | 阵法与部署          | 保留阵元、流向、供给与反制，并使用术语表显示。                |
| 泛化 `bindings`                   | 补充叙事关系         | 不再承担资源、解锁、突破、上限和转换等核心规则。               |

| `scriptures` / `manuals` | 物品库中的典籍物品 | 修行生态不建立典籍实体；法门 `itemIds`、能力 `scriptureSource.itemIds`、阵法 `itemIds` 只保存物品库稳定 ID。 |
| 体系内多轨道无关联字段 | 多轨道交叉规则（§6.4） | 若 V4 已有多轨道但无交叉约束，迁移时以空列表占位，不强制补填，审查器会给出建议提示。 |
| `characters[].inventory` | 角色库 / 物品库 | 由既有角色库和物品库负责，修行生态只做资产引用校验，不迁移或复制角色持有状态。 |

迁移必须兼容既有 V4 文件：缺少的新字段采用可验证默认值；无法自动判定语义的 `principles`、`conditions` 与泛化关系应进入迁移待处理清单，由作者或审查器确认归属。
...
角色修行档案不属于修行生态的数据模型，也不在本工作台中创建或维护。角色当前境界、法门掌握、能力习得、典籍研读和内部能量储量由角色库负责；修行生态只定义可被角色引用的体系资产及其规则。以下内容仅保留为跨模块职责说明，不能作为 `cultivation-ecology.json` 的字段。

...
- AI 只能创建带正文证据的 `proposed` 批次，证据必须是分析时正文中可逐字定位的连续原文片段，禁止概括、改写或拼接多处文本。分析结果必须在创建批次前使用与应用阶段相同的证据规则过滤；全部变化均无有效证据时分析失败，不得生成一个必然无法应用的批次。作者审阅后才可标记 `applied`；应用前仍须校验正文哈希与逐项证据，旧批次中的无效证据项默认取消选择且不得阻断同批有效项。删除章节按批次逆序标记 `reverted`，恢复章节可重新应用原批次。批次领域覆盖时间线、人物出场 / 状态 / 关系、物品、地点、势力、伏笔、世界规则和承接事项。正文叙事顺序、章节序号和模型猜测的自然语言日期不得作为世界时间；需要把正文事实送入世界推演时，批次必须引用已确认的正式时间线事件作为可选 `timeAnchorEventId`，无锚点或锚点时间未知的批次保留账本但不能编译为推演事实。当前一个批次只有一个时间锚点，多世界时间事件应拆批处理；旧批次缺省该字段时按未定时兼容。`novel_continuity_get_context` 默认只返回批次摘要和当前连续性事实，传 `chapterId` 可筛选章节，只有传 `batchId` 才展开单个完整批次，避免把全部证据和回滚快照一次塞入模型上下文。
...
- 线路、故事弧、卷篇组目录和章节计划可选保存 `simulationConstraint`：时间窗、必需/禁止事件或领域命令谓词、必需主体/地域和灵活度。该字段属于剧情工程自身的结构化设计，世界推演只在编译基线时复制并保留 `sourceRefs`，不得在运行期间反向修改剧情工程。
...
- 删除分支后，manifest 不再引用的文件可以作为提交后的清理项删除；清理失败只允许留下不可见孤立文件，不得使新的正式运行清单失效。V4 不读取、不迁移或导入任何旧式单文件 `run.json`、旧 `scenarios.json` 或旧运行目录；首次打开时若方案或运行索引明确标记为 V1-V3，Repository 会直接重建 V4 默认方案与空运行索引，并清理旧运行文件，不读取旧字段或恢复旧运行。结构损坏、缺失版本或未来版本仍必须报告格式错误。作者如需保留旧文件，应在工作台外自行备份。

### 21.1 世界推演 V4 入口与工作面

- 世界推演在小说工作台“辅助”菜单下只保留“世界推演”一个可见二级入口；点击后直接打开当前运行的统一推演工作台。没有当前运行时展示三栏空舞台，并提供单一“创建推演”动作，不保留世界推演首页、创建表单或内部菜单页。
- `simulation-console`、`simulation-lab`、`simulation-council` 是已废弃的内部路由，不得继续出现在 manifest、应用路由、导航或跳转逻辑中。因果链、会商、分支比较和采纳使用当前工作面内的弹窗或抽屉，而不是独立页面。
- 运行工作台采用左侧轮次时间线、中间文字叙事舞台、右侧实体状态检查器的单页布局；不嵌入或复刻世界地图。地图仍是独立一级模块，世界推演只消费其空间约束投影。
- 创建不要求作者填写任何运行参数。系统提供可选设置弹窗，最多调整推演名称、总时间范围、每轮跨度和叙事观察对象；不打开弹窗也会使用默认策略直接创建。事实截止点、范围、主体、人物初始地点、预算、随机种子、剧情工程观察和规则集版本由世界投影自动推导后随运行冻结。资料缺失时允许从世界日 0 或可得的最早事实继续。人物地点无法从已发生事实或人设资料推导时保持“未知”：该人物仍参与生命周期、关系与非空间行动，但不参与移动、地域遭遇等依赖明确地点的候选。内核仍须支持从天到项目历法可表达的超长跨度；观察镜头只控制叙事展开，不改变世界本身的推进。
- 世界架构、修炼、势力、资源和时间线设定必须先编译为运行的规则快照，再由确定性内核裁定。周期事件、生命周期、战争、资源争夺、人物遭遇与境界升级的执行顺序、长跨度聚合和 UI 行为以《世界推演设计方案》第 13、14、17 节为唯一细节权威。
- V4 不是旧世界推演的界面兼容层：旧内部菜单、旧页面、地图视图、旧运行数据和只为它们服务的无引用代码均应随正式实现删除。可复用的是仍符合 V4 契约的基线编译、确定性裁定、持久化、分支、会商和采纳领域能力，而非其旧 UI 或旧数据格式。
- 每轮是 `[from, to)` 的完整动态世界结算单位，必须保存事件、前后状态、规则命中、被拒绝候选、因果边、传播任务和叙事摘要；连续推演不得跳过中间轮次。
- 普通凡人在千年等长跨度下必须得到有证据的人生结局，叙事至少包含处境变化、关键选择或外部冲击、关系/民生后果和结局或镜头转移；可表现为安稳终老、疾病或意外、城镇遭屠戮、战争牵连、颠沛流离后病逝等。死亡后从行动候选移除，镜头可转移到后代、遗物、家族、地方记忆或长期制度影响；不能因观察对象选择而延寿，也不能只显示“寿命耗尽”。
- 秘境、节日、战争、资源争夺、人物遭遇和境界升级按《世界推演设计方案》规定的轮内顺序结算；长跨度按命中次数、异常、后果和代表事件聚合，不能逐年制造无意义事件。年度节日和千年秘境必须保留命中次数、受影响次数、关键异常与后续影响，支持从聚合结果下钻代表事件。
- 作者操作只产生观察、线索、护栏、会商或分支干预候选，确认后才影响未来轮次；已完成轮次和正式事实不可被直接编辑，推演文本不能新增未裁定事实。

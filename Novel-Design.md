...
- 叙事工程协议：`specs/tech_docs/novel_narrative_studio.md`
...
- 原“大纲”升级为“叙事设计”一级模块，同时保留 `outline/outline.md` 自由大纲视图；新增“灵感”和“创作方案”一级入口。
- 通用叙事内核只定义结构单元、叙事线路、故事弧、叙事节点、期待、情境、章节计划、检查项和关系。题材与项目专属术语只能来自创作方案，不得硬编码到通用组件。
- 正式事实源为 `story/narrative-design.json`、`inspiration/index.json` 和 `settings/creative-profile.json`，当前 `schemaVersion` 均为 `1`。三个 Repository 只依赖 `WorkbenchStorage`，保存统一使用 `expectedContent` 防止覆盖外部修改。
- 叙事对象关系必须区分计划锚点与已发生事实。结构父级不得形成循环；删除对象必须解除直接引用；正文章节继续由章节 Repository 拥有，章节计划只保存稳定引用。
- 故事弧支持任意数量阶段及结构、章节锚点。章节计划关联线路、故事弧、期待和创作方案声明的交付字段；字段控件支持文本、长文本、数字、开关、单选与多选。
- 期待统一承载承诺、悬念、谜团、伏笔和预言，阶段区分建立、强化、兑现、失效，并分别标记计划或已发生事实。
- 检查项可以来自创作方案或对象自定义，状态为待检查、已通过或手工豁免；手工豁免必须填写原因。验收视图按通用内核、配置层、项目规则或作者来源分组显示错误、警告和建议。
- 灵感保存项目独立副本，支持收集箱、待整理、未采用、已归档、列表、看板、搜索和排序。“已采用”只由 `adopted-as` 记录推导；同一灵感允许采用到多个叙事对象。
- 创作方案按“通用内核 -> 篇幅 -> 发布方式 -> 题材 -> 创作方法 -> 项目规则 -> 作者调整”确定性解析。配置只能声明术语、对象类型、字段、关系、检查项和视图，不执行项目 JavaScript。
- 直接重复稳定定义 ID 是阻断冲突。显式覆盖必须复用目标稳定 ID，并保持类别、作用对象与字段类型，避免已有项目数据失去映射。
- 当前不实现旧项目导入。导入必须在后续独立提供解析、映射、预览、冲突处理和作者确认，普通打开流程不得静默迁移或覆盖文件。

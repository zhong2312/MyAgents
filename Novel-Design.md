...
- 正式事实源为 `world/factions/index.json`，当前 `schemaVersion` 为 `1`。每个势力保存稳定 ID、名称、类型、状态、概要、地盘、成员、经营资产、争夺资源和创建/更新时间；保存必须使用 `expectedContent` 防止覆盖外部编辑。
...
- 经营资产保存名称、类型、规模或收益和说明；争夺资源保存名称、资源类型、控制状态和说明。它们是势力视角的事实，不替代物品库或世界架构中的原始资料。

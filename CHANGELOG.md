...
> My Novel Studio 0.5.0 将 MyAgents 的桌面工作台能力聚焦到小说创作。世界设定、修炼体系、人物、势力、物品、时间线、剧情工程和正文以同一小说项目目录为事实源，AI 协作始终通过可审阅的草稿与提案进入项目。

### Added

- **小说创作工作台**：补全世界架构、修炼体系、势力、物品、人物、时间线、剧情工程与正文创作的项目化工作流；各领域数据由对应 Repository 管理，Markdown 与 JSON 保持为可移动、可备份的事实源。
- **结构化领域校验与测试**：加强势力、时间线、叙事工程、物品与修炼体系的存储协议、跨领域引用校验及契约测试，避免不完整数据直接进入正式项目文件。
- **小说项目设置入口**：工作台标题栏提供设置入口，用于进入项目级模型场景等配置。
...
- **世界推演与小说工具链可靠性**：修复推演取消、提案处理、时间线与叙事上下文传递中的边界问题，保持作者确认前不直接改写正式项目数据。

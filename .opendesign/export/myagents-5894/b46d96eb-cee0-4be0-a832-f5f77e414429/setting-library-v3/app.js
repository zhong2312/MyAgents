...
const entriesBySetting = {
  "instance-celestial-river": [
    { id: "entry-celestial-river", name: "天河", category: "地理现象", aliases: "界河、银色水路", definition: "沿星海潮汐周期显形的银色水路，可在特定夜晚短暂连接彼此不相邻的世界。" },
    { id: "entry-qingyao-calendar", name: "青曜历", category: "历法", aliases: "青曜星历", definition: "以青曜恒星的光变与朔望交替记录航路周期的通行历法；第七夜是天河最稳定的显形窗口。" },
    { id: "entry-return-lamp", name: "返程舟灯", category: "器物", aliases: "出发地之灯、归航灯", definition: "船只进入天河前从出发地带走的灯火。灯焰用于识别返程水道，熄灭后船只将失去原世界坐标。" },
  ],
};
...
let editorView = "content";
let selectedEntryId = "entry-celestial-river";
...
function visualToMarkdown() {
  if (window.TurndownService) {
    ...
    converter.addRule("gfmTable", {
      filter: "table",
      replacement: (_content, node) => {
        const rows = Array.from(node.querySelectorAll("tr")).map((row) => Array.from(row.querySelectorAll("th, td")).map((cell) => cell.textContent.trim().replace(/\s+/g, " ").replace(/\|/g, "\\|")));
        if (!rows.length) return "";
        const width = Math.max(...rows.map((row) => row.length));
        const normalized = rows.map((row) => Array.from({ length: width }, (_, index) => row[index] || ""));
        const line = (cells) => `| ${cells.join(" | ")} |`;
        return `\n\n${line(normalized[0])}\n${line(Array(width).fill("---"))}${normalized.slice(1).map((row) => `\n${line(row)}`).join("")}\n\n`;
      },
    });
...
function insertTable() {
  ...
  if (editorMode === "source") {
    const source = byId("source-editor");
    const prefix = source.selectionStart > 0 && source.value[source.selectionStart - 1] !== "\n" ? "\n\n" : "";
    source.setRangeText(`${prefix}${markdown}\n\n`, source.selectionStart, source.selectionEnd, "end");
    source.focus();
  } else {
    byId("visual-editor").focus();
    document.execCommand("insertHTML", false, "<table><thead><tr><th>字段</th><th>内容</th></tr></thead><tbody><tr><td>示例</td><td>待填写</td></tr></tbody></table><p><br></p>");
  }
  persistEditorContent();
  showToast("已插入两列表格", "table-2");
}
  ...
  const renderNodes = (nodes, depth) => {
    nodes.forEach((node) => {
      ...
      if (match || !query) {
        ...
        const toggle = hasChildren ? `<button class="tree-toggle ${expanded ? "expanded" : ""}" type="button" data-tree-toggle aria-label="${expanded ? "折叠" : "展开"}${escapeHtml(node.name)}"><i data-lucide="chevron-right"></i></button>` : '<span class="tree-toggle placeholder" aria-hidden="true"></span>';
        rows.push(`<div class="tree-item" role="none"><div class="tree-row ${node.id === selectedNodeId ? "selected" : ""}" role="treeitem" aria-selected="${node.id === selectedNodeId}"${expandedAttribute} data-node-id="${escapeHtml(node.id)}" style="--tree-depth:${depth}">${toggle}<span class="tree-icon"><i data-lucide="${escapeHtml(type?.icon || "brackets")}"></i></span><span class="tree-name" title="${escapeHtml(node.name)}">${escapeHtml(node.name)}</span><span class="tree-type" title="层级类型：${escapeHtml(type?.name || "未配置")}">${escapeHtml(type?.name || "未配置")}</span></div></div>`);
      }
...
function renderSettings() {
  ...
  const renderRows = (items, source) => items.map((item) => {
    const sourceLabel = source === "default" ? "默认设定" : "自定义设定";
    const sourceIcon = source === "default" ? "layout-template" : "file-pen-line";
    return `<button class="setting-row ${item.ref === selectedSettingRef ? "selected" : ""}" type="button" data-setting-ref="${escapeHtml(item.ref)}"><span class="setting-icon" title="${sourceLabel}" aria-label="${sourceLabel}"><i data-lucide="${sourceIcon}" aria-hidden="true"></i></span><span class="setting-row-copy"><strong title="${escapeHtml(item.name)}">${escapeHtml(item.name)}</strong><small>${escapeHtml(item.group)} · ${item.kind === "virtual" ? "虚拟页面" : "Markdown"}</small></span>${settingStateMarkup(item.status)}</button>`;
  }).join("");
  byId("settings-node-meta").textContent = `${type?.name || "未配置"} · ${groups.defaults.length} 个默认模板`;
  byId("setting-list").innerHTML = `${renderRows(defaults, "default")}${renderRows(custom, "custom")}` || '<div class="settings-empty">当前节点暂无设定</div>';
  refreshIcons();
}
...
function entriesForCurrentSetting() {
  return entriesBySetting[selectedSettingRef] || [];
}

function ensureEntryStore() {
  if (!selectedSettingRef) return [];
  if (selectedSettingRef.startsWith("virtual:")) {
    const virtualRef = selectedSettingRef;
    const pendingEntries = entriesBySetting[virtualRef] || [];
    ensureInstanceFromVirtual();
    entriesBySetting[selectedSettingRef] ||= pendingEntries;
    if (virtualRef !== selectedSettingRef) delete entriesBySetting[virtualRef];
  }
  entriesBySetting[selectedSettingRef] ||= [];
  return entriesBySetting[selectedSettingRef];
}

function renderEntryEditor() {
  const entry = entriesForCurrentSetting().find((item) => item.id === selectedEntryId);
  byId("entry-empty").classList.toggle("hidden", Boolean(entry));
  byId("entry-form").classList.toggle("hidden", !entry);
  if (!entry) return;
  const settingName = currentInstance()?.name || currentVirtualTemplate()?.name || "当前设定";
  byId("entry-context").textContent = `${currentNode().name} · ${settingName}`;
  byId("entry-editor-title").textContent = entry.name || "未命名词条";
  byId("entry-name").value = entry.name;
  byId("entry-category").value = entry.category;
  byId("entry-aliases").value = entry.aliases;
  byId("entry-definition").value = entry.definition;
}

function renderEntries() {
  const query = byId("entry-search")?.value.trim().toLowerCase() || "";
  const entries = entriesForCurrentSetting();
  if (!entries.some((entry) => entry.id === selectedEntryId)) selectedEntryId = entries[0]?.id || "";
  const filtered = entries.filter((entry) => `${entry.name}${entry.category}${entry.aliases}${entry.definition}`.toLowerCase().includes(query));
  byId("entry-count").textContent = entries.length;
  byId("entry-list").innerHTML = filtered.map((entry) => `<button class="entry-row ${entry.id === selectedEntryId ? "selected" : ""}" type="button" aria-current="${entry.id === selectedEntryId}" data-entry-id="${escapeHtml(entry.id)}"><span class="entry-icon"><i data-lucide="notebook-text"></i></span><span class="entry-row-copy"><strong>${escapeHtml(entry.name || "未命名词条")}</strong><small>${escapeHtml(entry.category)}${entry.aliases ? ` · ${escapeHtml(entry.aliases)}` : ""}</small></span></button>`).join("") || `<div class="entry-list-empty">${entries.length ? "没有匹配的词条" : "暂无词条"}</div>`;
  renderEntryEditor();
  refreshIcons();
}

function setEditorView(view, focusTab = false) {
  editorView = view;
  ...
  byId("editor-content-panel").classList.toggle("hidden", view !== "content");
  byId("editor-entry-panel").classList.toggle("hidden", view !== "entries");
  if (view === "entries") renderEntries();
}

function createEntry() {
  if (!selectedSettingRef) return;
  const entries = ensureEntryStore();
  const entry = { id: `entry-${Date.now()}`, name: "未命名词条", category: "术语", aliases: "", definition: "" };
  entries.push(entry);
  selectedEntryId = entry.id;
  renderEntries();
  byId("entry-name").focus();
  byId("entry-name").select();
  markDirty();
}
...
function updateSelectedEntry(field, value) {
  ...
  if (!entry) return;
  entry[field] = value;
  if (field === "name") byId("entry-editor-title").textContent = value || "未命名词条";
  markDirty();
}
...
function deleteSelectedEntry() {
  ...
  selectedEntryId = entries[Math.min(index, entries.length - 1)]?.id || "";
  renderEntries();
  showToast(`已删除词条“${removed.name}”`, "trash-2");
  markDirty();
}
...
function openMeta(tab = "types") {
  ...
  syncSectionNavigation("templates");
  selectMetaTab(tab);
  renderAllMeta();
}
...
function syncSectionNavigation(route) {
  document.querySelectorAll("[data-section-route]").forEach((button) => {
    const active = button.dataset.sectionRoute === route;
...
function bindEvents() {
  ...
  byId("sort-settings").addEventListener("click", () => {
    ...
    byId("sort-settings").setAttribute("aria-pressed", String(settingsSortAlpha));
    byId("sort-settings").setAttribute("aria-label", settingsSortAlpha ? "恢复模板顺序" : "按名称排序设定");
    byId("sort-settings").title = settingsSortAlpha ? "恢复模板顺序" : "按名称排序";
    renderSettings();
  ...
  document.querySelectorAll("[data-section-route]").forEach((button) => button.addEventListener("click", () => {
    button.dataset.sectionRoute === "templates" ? openMeta("templates") : closeMeta();
  }));
  ...
  byId("rail-toggle").addEventListener("click", () => showToast("设定导航已固定在工作台菜单内", "panel-left"));
  byId("project-switcher").addEventListener("click", () => showToast("当前项目：烬海编年史", "book-open"));

  document.querySelectorAll("[data-editor-view]").forEach((tab) => tab.addEventListener("click", () => setEditorView(tab.dataset.editorView)));
  byId("editor-view-tabs").addEventListener("keydown", (event) => {
    const tabs = Array.from(document.querySelectorAll("[data-editor-view]"));
    const currentIndex = tabs.indexOf(document.activeElement);
    if (currentIndex < 0 || !["ArrowLeft", "ArrowRight", "Home", "End"].includes(event.key)) return;
    event.preventDefault();
    const nextIndex = event.key === "Home" ? 0 : event.key === "End" ? tabs.length - 1 : (currentIndex + (event.key === "ArrowRight" ? 1 : -1) + tabs.length) % tabs.length;
    setEditorView(tabs[nextIndex].dataset.editorView, true);
  });
  byId("entry-search").addEventListener("input", renderEntries);
  byId("add-entry").addEventListener("click", createEntry);
  byId("entry-list").addEventListener("click", (event) => {
    const row = event.target.closest("[data-entry-id]");
    if (!row) return;
    selectedEntryId = row.dataset.entryId;
    renderEntries();
  });
  byId("entry-form").addEventListener("submit", (event) => event.preventDefault());
  byId("entry-name").addEventListener("input", (event) => updateSelectedEntry("name", event.target.value));
  byId("entry-category").addEventListener("change", (event) => {
    updateSelectedEntry("category", event.target.value);
    renderEntries();
  });
  byId("entry-aliases").addEventListener("input", (event) => updateSelectedEntry("aliases", event.target.value));
  byId("entry-definition").addEventListener("input", (event) => updateSelectedEntry("definition", event.target.value));
  byId("entry-name").addEventListener("change", renderEntries);
  byId("entry-aliases").addEventListener("change", renderEntries);
  byId("delete-entry").addEventListener("click", deleteSelectedEntry);
  ...
  byId("insert-table").addEventListener("mousedown", (event) => {
    event.preventDefault();
    insertTable();
  });

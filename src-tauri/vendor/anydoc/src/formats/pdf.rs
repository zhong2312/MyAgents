...
pub fn to_markdown(bytes: &[u8]) -> Result<String, ConvertError> {
    ...
    if !result.pages_needing_ocr.is_empty() {
        recovery_warn!(
            "DOCUMENT_PDF_PAGE_SKIPPED",
            Some(format!("pages {:?}", result.pages_needing_ocr)),
    ...
    if result.has_encoding_issues {
        recovery_warn!(
            "DOCUMENT_PDF_ENCODING_RECOVERED",
            None,
            "broken font encodings detected; extracted text may be garbled"
        );
    }
